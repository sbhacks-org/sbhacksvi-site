...
let renderText = renderTemplate("decision_notification.txt.ejs");
let renderHTML = renderTemplate("decision_notification.ejs");
		...
		rl.question(`Sending out ${emails.length} email(s). Would you like to continue? (y/n): `, (answer) => {
			if(answer == "y") {
				const message = {
					...
					subject: "SB Hacks IV First Round Decisions",
					text: content[0],
					html: content[1]
				};
