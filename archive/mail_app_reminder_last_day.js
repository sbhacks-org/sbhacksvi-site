...
let renderText = renderTemplate("day_of_app_reminder.txt.ejs");
let renderHTML = renderTemplate("day_of_app_reminder.ejs");
		...
		rl.question(`Sending out ${emails.length} email(s). Would you like to continue? (y/n): `, (answer) => {
			if(answer == "y") {
				const message = {
					...
					subject: "SB Hacks V Applications close TONIGHT!",
					text: content[0],
					html: content[1]
				};
